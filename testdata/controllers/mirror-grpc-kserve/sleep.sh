...
data:
  ...
  query.sh: |
    echo "cat input.json | grpcurl -plaintext -proto kserve.proto -d @ wisdom.default.svc.cluster.local:80 inference.GRPCInferenceService.ModelInfer"
    cat input.json | grpcurl -plaintext -proto kserve.proto -d @ wisdom.default.svc.cluster.local:80 inference.GRPCInferenceService.ModelInfer
EOF
