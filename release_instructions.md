...
1. Go to the `master` branch and update the releases page and the redirects file
2. Create a new branch off of `master` using the format `release-<release version>`. For example: `release-0.2.1` or `release-1.0.0`.
3. Follow the [Create a new site](#create-a-new-site) instructions and use the new branch to create a new site on Netlify with the [iter8.tool](iter8.tool) and archival (e.g. [v0-2-1.iter8.tools](v0-2-1.iter8.tools)) domains. Ensure that the build command uses the Hugo `-b` or `--baseURL` to point to the archival domain.
4. Change Netlify site name using the format `iter8-<release version>`. For example: `iter8-v0-2-1` or `iter8-v1-0-0`.
5. Change the `versionNumber` and `versionName` in the `config.toml` and commit it.
6. Change the badges at tge top of the `README.md` and commit it. `git cherry-pick` the commit to the `master` branch as well.

Changes for the old latest site:

1. Go to `Domain management` of [iter8.tools](iter8.tools).
2. An archival domain should already exist (e.g. [v0-2-1.iter8.tools](v0-2-1.iter8.tools)). Set the archival domain as the primary domain.
3. Remove the [iter8.tools](iter8.tools) domain.
4. Remove the `static/_redirects` file.
