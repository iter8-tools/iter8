...
import (
	...
	//RR "github.com/iter8-tools/iter8/base"
...
type KubeDriver struct {
	...
	*action.Configuration
	...
	Revision int
	// DynamicClient enables unstructured queries
	DynamicClient dynamic.Interface
	// Mapper used to find GVR
	Mapper *restmapper.DeferredDiscoveryRESTMapper
}

// NewKubeDriver creates and returns a new KubeDriver
func NewKubeDriver(s *MyEnvSettings) *KubeDriver {
	kd := &KubeDriver{
		MyEnvSettings: s,
...
func (kd *KubeDriver) initKube() (err error) {
	if kd.Clientset == nil {
		...
		if err != nil {
			e := errors.New("unable to get Kubernetes discovery client")
			log.Logger.WithStackTrace(err.Error()).Error(e)
			return e
		}
		...
		if err != nil {
			e := errors.New("unable to get Kubernetes dynamic interface")
			log.Logger.WithStackTrace(err.Error()).Error(e)
			return e
		}
	}
...
// 	ch, vals, err := getChartAndVals(cpo, chartName, driver.MyEnvSettings, valueOpts)
// 	if err != nil {
// 		e := fmt.Errorf("unable to get chart and vals for %v", chartName)
// 		log.Logger.WithStackTrace(err.Error()).Error(e)
// 		return e
...
// // Credit: the logic for this function is sourced from Helm
// // https://github.com/helm/helm/blob/8ab18f7567cedffdfa5ba4d7f6abfb58efc313f8/cmd/helm/install.go#L177
// func getChartAndVals(cpo *action.ChartPathOptions, chartName string, settings *MyEnvSettings, valueOpts values.Options) (*chart.Chart, map[string]interface{}, error) {
