...
## Canary testing
Two app versions, a `baseline` and a `candidate`, are evaluated in canary tests. Candidate is declared the winner if it satisfies experiment objectives. If candidate fails to satisfy objectives but baseline does, then baseline is declared the winner.

## Conformance testing
A single app version is evaluated in conformance tests; it is declared the winner if it satisfies experiment objectives.