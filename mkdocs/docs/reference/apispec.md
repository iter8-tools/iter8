...
!!! abstract "Abstract"
    ...
    1. The `Experiment` object provides expressive controls required by application developers and service operators who wish to automate new releases of their apps in a robust, principled and metrics-driven manner. These controls encompass [testing, deployment, traffic shaping, and version promotion functions](/concepts/experimentationstrategies/) and can be flexibly composed to automate [diverse use-cases](/code-samples/knative/canary-progressive/).
