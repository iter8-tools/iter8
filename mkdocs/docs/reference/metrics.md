...
| level | string | Level of a version. The semantics of level are as follows. If the metric is a counter, if level is `x`, and time elapsed since the start of the experiment is `y`, then `xy` is the metric value. This will keep increasing the metric value over time. If the metric is gauge, if level is `x`, the metric value is a random value with mean `x`. The expected value of the metric will be `x` but its observed value may increase or decrease over time. | Yes |
