...
replacing `[YOUR_ORG]` with your Github organization name, aka, your Github username. Now, do the same replacement operation to update some references in the repo so they will point at your forked repo.
...
??? info "More about Iter8 GitOps handler task"
    Iter8 operating in the GitOps mode is very similar to how it works normally. One key difference is that at the end of the experiment, it will need to perform an additional step to modify the desired state of the Env repo to reflect the outcome of the experiment. This can be done by specifying a `finish` Iter8 handler task that runs at the end of an experiment. The specific handler task we are using in this tutorial is written in a shell script as shown below:
    ```yaml linenums="1"
    actions:
