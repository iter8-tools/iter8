...
*match* | -- | Match rules used to filter out incoming traffic. With protocol name as a key, its value is an array of istio matching clauses. Currently only `http` is supported. ([istio http match rules](((https://istio.io/latest/docs/reference/config/networking/virtual-service/#HTTPMatchRequest))) | no
